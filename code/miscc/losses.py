...
def discriminator_loss(netD, real_imgs, fake_imgs, conditions,
                       ...
                       transf_matrices=None, transf_matrices_inv=None):
    # Forward
    # real_features = netD(real_imgs)
    # fake_features = netD(fake_imgs.detach())
    if local_labels is not None:
        inputs = (real_imgs, local_labels, transf_matrices, transf_matrices_inv)
    else:
        inputs = (real_imgs)
    ...
    if local_labels is not None:
        inputs = (fake_imgs.detach(), local_labels, transf_matrices, transf_matrices_inv)
    else:
        inputs = (fake_imgs.detach())
    fake_features = nn.parallel.data_parallel(netD, inputs, gpus)
...
def generator_loss(netsD, image_encoder, fake_imgs, real_labels,
                   ...
                   cap_lens, class_ids, gpus, local_labels=None,
                   transf_matrices=None, transf_matrices_inv=None):
    numDs = len(netsD)
    ...
    for i in range(numDs):
        ...
        if i == 0:
            inputs = (fake_imgs[i], local_labels, transf_matrices, transf_matrices_inv)
        else:
            inputs = (fake_imgs[i])
...
def VQA_loss(VQA_model, img_feats, question_programs, mapped_ans, gpus):
    xent_loss = nn.CrossEntropyLoss()
    question_programs = question_programs.detach()
    outs = VQA_model(img_feats, question_programs)
    # _, preds = outs.max(1)
    loss = xent_loss(outs, mapped_ans)
    return loss
