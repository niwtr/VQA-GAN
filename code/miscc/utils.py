...
def render_PIL_image_b64(pil_image):
...
def build_super_images(real_imgs, captions, ixtoword,
                       attn_maps, att_sze, lr_imgs=None,
                       batch_size=cfg.TRAIN.BATCH_SIZE,
                       max_word_num=cfg.TEXT.WORDS_NUM, nvis = 8):
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            if (vis_size // att_sze) > 1:
                one_map = \
                    skimage.transform.pyramid_expand(one_map, sigma=8,
...
def tarball_directory(tarball_path, d, exclude_pred= lambda f: f.endswith('.pyc')):
    import tarfile
    tarball = tarfile.open(tarball_path, 'w:gz')
    tarball.add(d, exclude= exclude_pred)
    tarball.close()

#############################
def save_pure_img_results(data_img, fake, step, image_dir, vis_num = 64, token = ''):
    num = vis_num if vis_num < cfg.TRAIN.BATCH_SIZE else cfg.TRAIN.BATCH_SIZE
    fake = fake[0:num]
    # data_img is changed to [0,1]
    if data_img is not None:
        data_img = data_img[0:num]
        vutils.save_image(
            data_img, '%s/%s_real_samples_step_%03d.png' % 
            (image_dir, token, step), normalize=True)
        # fake.data is still [-1, 1]
        vutils.save_image(
            fake.data, '%s/%s_fake_samples_step_%03d.png' %
            (image_dir, token, step), normalize=True)
    else:
        vutils.save_image(
            fake.data, '%s/%s_lr_fake_samples_step_%03d.png' %
            (image_dir, token, step), normalize=True)
'''
Save a batch of images into specific path.
@param img: batch of images
@filenames: batch of filenames
@directory: where to save those images.
'''
# def save_batch_images(img, filenames, directory):
#     for im, key in zip(img, filenames):
#         vutils.save_image(im, os.path.join(directory, key), normalize=True)
#     return
def save_batch_images(img, filenames, directory):
    bsz = img.size(0)
    for im, key in zip(img, filenames):
        im_cpu = im.cpu().numpy()
        im_cpu = (im_cpu + 1.0) * 127.5
        im_cpu = im_cpu.astype(np.uint8)
        im_cpu = np.transpose(im_cpu, (1,2,0))
        im_pil = Image.fromarray(im_cpu)
        im_pil.save(os.path.join(directory, key))
