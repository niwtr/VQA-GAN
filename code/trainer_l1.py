...
from datasets import prepare_data as prepare_data_coco
...
class condGANTrainer(object):
    def __init__(self, output_dir, data_loader, n_words, ixtoword, resume):
        ...
        if 'clevr' in cfg.DATASET_NAME:
            ...
            self.prepare_data = prepare_data_coco
    ...
    def build_models(self):
        ...
        img_encoder_path = cfg.TRAIN.NET_E.replace('qa_encoder', 'image_encoder')
        state_dict = \
        ...
        qa_encoder = RNN_ENCODER(self.n_words, nhidden=cfg.TEXT.EMBEDDING_DIM)
        state_dict = torch.load(cfg.TRAIN.NET_E, map_location=lambda s, l: s)
        qa_encoder.load_state_dict(state_dict)
        for p in  qa_encoder.parameters():
            p.requires_grad = False
        print('Loaded qa encoder: %s' % cfg.TRAIN.NET_E)
        qa_encoder.eval()
        ...
        # ########################################################### #
        if cfg.CUDA:
            qa_encoder = qa_encoder.cuda()
            image_encoder = image_encoder.cuda()
            netG.cuda()
            for i in range(len(netsD)):
                netsD[i].cuda()
        return [qa_encoder, image_encoder, netG, netsD, epoch]
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                per_qa_embs, avg_qa_embs, qa_nums =\
                    Level1RNNEncodeMagic(captions, cap_lens, text_encoder)
    ...
    def sample_full_split(self, data_loader, split = 'train'):
        ...
        image_encoder.eval()
        netG.eval(); del netsD
        save_dir = cfg.TRAIN.NET_G[:cfg.TRAIN.NET_G.find('.pth')] + "_samples_" + split
        ...
        for i, data in enumerate(data_loader, 0):
            with torch.no_grad():
                ...
                fake_imgs, _, mu, logvar = nn.parallel.data_parallel(netG, inputs, self.gpus)
                level = -1
                save_batch_images(fake_imgs[-1], keys, save_dir)
                # self.image_dir = '/home/ntr/tmp/'
                # self.save_img_results(imgs, netG, noise, avg_qa_embs,
                #                       per_qa_embs, mask, image_encoder,
