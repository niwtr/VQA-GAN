...
def train(dataloader, cnn_model, rnn_model, batch_size,
    ...
    global global_step
    H_rnn_model, L_rnn_model = rnn_model
    cnn_model.train()
    H_rnn_model.train()
    L_rnn_model.train()
    s_total_loss0 = 0
    ...
    for step, data in enumerate(dataloader, 0):
        global_step += 1
        # print('step', step)
        rnn_model[0].zero_grad()
        rnn_model[1].zero_grad()
        cnn_model.zero_grad()
        ...
        per_qa_embs, avg_qa_embs, num_caps =\
        ...
        torch.nn.utils.clip_grad_norm(L_rnn_model.parameters(),
                                      cfg.TRAIN.RNN_GRAD_CLIP)
        torch.nn.utils.clip_grad_norm(H_rnn_model.parameters(),
                                      cfg.TRAIN.RNN_GRAD_CLIP)
        optimizer.step()
...
def build_models(dataset):
    # build model ############################################################
    text_encoder_L = RNN_ENCODER(dataset.n_words, nhidden=cfg.TEXT.EMBEDDING_DIM)
    ...
    if cfg.TRAIN.NET_E != '':
        ...
        print('Load ', name)
        # 
        name = cfg.TRAIN.NET_E.replace('text_encoder', 'text_encoder_L')
        state_dict = torch.load(name)
    ...
    return (text_encoder, text_encoder_L), image_encoder, labels, start_epoch
...
def main():
    ...
    (higher_text_encoder, sub_text_encoder) = text_encoder
    para = list(higher_text_encoder.parameters()) + list(sub_text_encoder.parameters())
    ...
    try:
        ...
        for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
            ...
            if (epoch % cfg.TRAIN.SNAPSHOT_INTERVAL == 0 or
                ...
                torch.save(higher_text_encoder.state_dict(),
                           '%s/text_encoder%d.pth' % (model_dir, epoch))
                torch.save(sub_text_encoder.state_dict(),
                           '%s/text_encoder_L%d.pth' % (model_dir, epoch))
                print('Saved models.')
